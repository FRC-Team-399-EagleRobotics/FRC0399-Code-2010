...
public class CANJaguar implements JaguarCANProtocol, SpeedController {
    ...
    /**
     * Determines how the Jaguar behaves whne sending a zero signal.
     */
    ...
    public static class Faults {
        ...
        public final int value;
        public static final int kCurrentFault_val = 1;
        public static final int kTemperatureFault_val = 2;
        public static final int kBusVoltageFault_val = 4;
    ...
    private void initJaguar() {
        ...
        switch (m_controlMode.value) {
            case ControlMode.kPercentVoltage_val:
                message = LM_API_VOLT_T_EN;
                break;
            case ControlMode.kCurrent_val:
                message = LM_API_ICTRL_T_EN;
                break;
            case ControlMode.kPosition_val:
                message = LM_API_POS_T_EN;
                break;
            case ControlMode.kSpeed_val:
                message = LM_API_SPD_T_EN;
                break;
            default:
                return;
        }
        sendMessage(message | m_deviceNumber, kNoData, 0);
    }
    ...
    protected byte receiveMessage(int messageID, byte[] data, double timeout) {
        ...
        byte dataSize = 0;
        try{
        dataSize = canDriver.receiveMessage(messageID, data, timeout);
        }catch(UncleanStatusException unclean){
            //Jaguar error on CAN bus
            System.err.println("UncleanStatusException : " + unclean);
        }
        return dataSize;
    }
...
//            dataSize = receiveMessage(ackMessageID, kNoData, 0.0);
    ...
    public long getFirmwareVersion() {
        ...
        dataSize = getTransaction(CAN_MSGID_API_FIRMVER, dataBuffer);
        if (dataSize == 4) {
